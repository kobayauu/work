...
#endif
	::_tzset();
	SetRegistryKey( _T("Brother Industries,Ltd.\\GTUpgradeTools") );
...
/*	if( csCmdLine.Find( _T("-unit") ) >= 0 ){
	...
	/* ミューテックスを作成する */
	HANDLE hMutex;
	hMutex = CreateMutex(NULL, TRUE, _T("GTX6SBUPGRADETOOL"));
...
#define MAINTE_TOOL_PATH "\\Brother\\GTX-6 Tools\\GTX6Maintenance.exe"
...
int CBGJServToolApp::GetGTPrinters( CComboBox* pPrinterComBox )
{
	...
	auto type = (GTX_PRINTER_TYPE)(TYPE_GTX6|TYPE_USB|TYPE_LAN);
...
// 小林追加
#define FIRM_UPDATE_GTX6_PATH   "\\Brother\\GTX-6 Tools\\GTX6FirmwareUpdate.exe"
#define FIRM_UPDATE_GTX6SP_PATH "\\Brother\\GTX-6 SP Tools\\GTX6SPFirmwareUpdate.exe"

void CBGJServToolApp::RunFirmUpdate_GTX6()
{
	CString strFirmUpdatePath;

	int iRet = GetProgramFilesPath(&strFirmUpdatePath);

	if (iRet >= 0)
	{
		strFirmUpdatePath += FIRM_UPDATE_GTX6_PATH;

		ShellExecute(NULL, _T("open"), strFirmUpdatePath, _T(""), NULL, SW_HIDE );
	}

}

void CBGJServToolApp::RunFirmUpdate_GTX6SP()
{
	CString strFirmUpdatePath;
	...
	if (iRet >= 0)
	{
		strFirmUpdatePath += FIRM_UPDATE_GTX6SP_PATH;
...
int CBGJServToolApp::FetchFirmwareVersion(DWORD* pdwFirmVer)
{
	...
	char const* status = gtbidi_fetch_firmware_version_for_gtx6(p, &err);
	gtbidi_close_printer(p);
	if (err.code != GTBIDI_SUCCESS) {
		return err.code;
	}

	nlohmann::json j = nlohmann::json::parse(status);
	gtbidi_free_string(status);

	auto get_version = [&j](LPCSTR name) {
		if (j.count(name) == 0)	return -1LL;
		auto value = j[name].get<std::string>();
		return std::stoll(value.erase(5, 1).erase(2, 1), nullptr, 16);
	};

	pdwFirmVer[0] = get_version("main_pcb_firmware");
	pdwFirmVer[1] = get_version("actuator_pcb_firmware_1");
	pdwFirmVer[2] = get_version("actuator_pcb_firmware_2");
	pdwFirmVer[3] = get_version("actuator_pcb_firmware_3");
	pdwFirmVer[4] = get_version("message_data");
	pdwFirmVer[5] = get_version("test_print_data");
	pdwFirmVer[6] = get_version("head_firmware_color_a");
	pdwFirmVer[7] = get_version("head_firmware_color_b");
	pdwFirmVer[8] = get_version("head_firmware_white_a");
	pdwFirmVer[9] = get_version("head_firmware_white_b");
	pdwFirmVer[10] = get_version("lcd_firmware");

	return	GTBIDI_SUCCESS;
}

int CBGJServToolApp::CheckVersion(DWORD dwVersion)
{
	int	iRet = 0;
	DWORD dwFirmVer[13];

	//本体バージョン取得
	iRet = FetchFirmwareVersion(dwFirmVer);
	if (iRet != GTBIDI_SUCCESS) {
		return iRet;
	}

	//バージョン比較
	if (dwFirmVer[0] >= dwVersion) {
		iRet = 0;
	}
	else {
		iRet = -1;
