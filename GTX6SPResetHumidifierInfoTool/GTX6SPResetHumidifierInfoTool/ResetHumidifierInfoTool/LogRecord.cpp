...
void CLogAnalyze::getCountDotSp(unsigned long dot[])
{
	int cnt = 0;
	CString strText = m_addition;
	...
	while(cnt < 42)
	{
		...
		{
			CString strDot = "0x" + strToken;
			CLogString::stringToDigit(strDot, dot[cnt-12]);
		}
		cnt++;
	}
}
...
BOOL CLogAnalyze::GetJobAllSp( CString* pPlaten, CString* pArpTime, CString* pMachine, CString* pTranscolor,
							char* pSatu, char* pBrig, char* pCont, char* pInkVolume,
							char* pUniDir, char* pBlack, char* pMask, char* pHighlight, char* cMinWhite )
{
	if(!m_addition.IsEmpty())
	{
		...
		*pPlaten			=		CLogString::tokenizeString( strText, " ", start, 15-13 ); //[13][14]プラテン名
									...
									CLogString::tokenize( strText, " ", start );	//[39]
									CLogString::tokenize( strText, " ", start );	//[40]
									CLogString::tokenize( strText, " ", start );	//[41]
									CLogString::tokenize( strText, " ", start );	//[42]
									CLogString::tokenize( strText, " ", start );	//[43]
									CLogString::tokenize( strText, " ", start );	//[44]

		CString strToken45 = "0x" + CLogString::tokenize( strText, " ", start );	//[45]ドライバ設定 n 1- 4
		CString strToken46 = "0x" + CLogString::tokenize( strText, " ", start );	//[46]ドライバ設定 n 5- 8
		CString strToken47 = "0x" + CLogString::tokenize( strText, " ", start );	//[47]ドライバ設定 n 9-12
		CString strToken48 = "0x" + CLogString::tokenize( strText, " ", start );	//[48]ドライバ設定 n13-16
		CString strToken49 = "0x" + CLogString::tokenize( strText, " ", start );	//[49]ドライバ設定 n17-20
		CString strToken50 = "0x" + CLogString::tokenize( strText, " ", start );	//[50]ドライバ設定 n21-24
		CString strToken51 = "0x" + CLogString::tokenize( strText, " ", start );	//[51]ドライバ設定 n25-28
		CString strToken52 = "0x" + CLogString::tokenize( strText, " ", start );	//[52]ドライバ設定 n29-32

		unsigned long digit2, digit3, digit4, digit5, digit6, digit7, digit8, digit9;
		CLogString::stringToDigit( strToken45, digit2 );	//２番目の４バイト	1-4
		CLogString::stringToDigit( strToken46, digit3 );	//３番目の４バイト	5-8
		CLogString::stringToDigit( strToken47, digit4 );	//４番目の４バイト	9-12
		CLogString::stringToDigit( strToken48, digit5 );	//５番目の４バイト	13-16
		CLogString::stringToDigit( strToken49, digit6 );	//６番目の４バイト	17-20
		CLogString::stringToDigit( strToken50, digit7 );	//７番目の４バイト	21-24
		CLogString::stringToDigit( strToken51, digit8 );	//８番目の４バイト	25-28
		CLogString::stringToDigit( strToken52, digit9 );	//９番目の４バイト	29-32
